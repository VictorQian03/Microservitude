...
class CostCalculationError(ValueError): pass

def calculate_pct_adv_cost(
    *, notional_usd: Decimal, adv_usd: Decimal, c: Decimal, cap: Decimal | None = None
) -> Tuple[Decimal, Decimal]:
    if notional_usd <= ZERO: raise CostCalculationError("Notional must be positive.")
    if adv_usd     <= ZERO: raise CostCalculationError("ADV must be positive.")
...
def calculate_sqrt_cost(
    *, shares: int | Decimal, adv_shares: Decimal, price: Decimal, a: Decimal, b: Decimal
    ...
    if s          <= ZERO: raise CostCalculationError("Shares must be positive.")
    if adv_shares <= ZERO: raise CostCalculationError("ADV shares must be positive.")
    if price      <= ZERO: raise CostCalculationError("Price must be positive.")
